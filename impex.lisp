...
(defcfun ("vigra_exportgrayimage_c" vigra_exportgrayimage_c) :int
	...
	(height :int)
	(filename :string)
	(rescale_range :boolean))

(defun savegrayimage (image filename &optional (rescale_range T))
  	(let* ((band   (first image))
	 	   (width  (band-width band))
	 	   (height (band-height band))
		   (result (with-array-as-foreign-pointer
	 					(band ptr_band :float :lisp-type single-float) 
	 			 		(vigra_exportgrayimage_c ptr_band width height filename rescale_range))))
    	(case result
      		((0) t)
...
(defun savergbimage (image filename &optional (rescale_range T))
 	 (let* ((band_r  (first image))
			...
			(result (with-arrays-as-foreign-pointers
						((band_r ptr_band_r :float :lisp-type single-float) 
						 (band_g ptr_band_g :float :lisp-type single-float) 
						 (band_b ptr_band_b :float :lisp-type single-float))
						(vigra_exportrgbimage_c ptr_band_r ptr_band_g ptr_band_b width height filename rescale_range))))
		(case result
			((0) T)
...
(defun savergbaimage (image filename &optional (rescale_range T))
 	 (let* ((band_r  (first image))
	 		(band_g  (second image))
		 	(band_b  (third image))
		 	(band_a  (fourth image))
			...
			(result (with-arrays-as-foreign-pointers
						((band_r ptr_band_r :float :lisp-type single-float) 
						 ...
						 (band_b ptr_band_b :float :lisp-type single-float) 
						 (band_a ptr_band_a :float :lisp-type single-float))
						(vigra_exportrgbaimage_c ptr_band_r ptr_band_g ptr_band_b ptr_band_a width height filename rescale_range))))
		(case result
			((0) T)
			((1) (error "Error in vigracl.impex.savergbaimage: Image cannot be saved by vigra!")))))

;######    Generic (choose automatically if image is gray or colored)    #######
(defun saveimage (image filename &optional (rescale_range T))
  	(case (length image)
...
(defun save-image  (image filename &optional (rescale_range T)) (saveimage image filename rescale_range))
(defun image-save  (image filename &optional (rescale_range T)) (saveimage image filename rescale_range))
